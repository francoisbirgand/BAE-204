...
The theoretical wetland soil used to illustrate our point has `enough'
organic matter content for microbial respiration to take place
throughout its profile, `enough' of other sand, silt, and clay and all
the minerals that accompany them, including iron and manganese oxides.
Let us assume that this theoretical soil, is sufficiently moist and
aerated for microbial aerobic respiration to take place throughout the
soil profile. Let us then assume that this soil is suddenly flooded, and
let us explore the consequences of this.

\subsection{An aerobic layer near the soil-water
...
aerobic and anaerobic zones of a theoretical flooded wetland soil due to
the imbalance between bacterial respiratory oxygen demand and oxygen
...
\subsection{Respiration in the anaerobic zone of the
...
\emph{spatial} sequence or zones where each of the electron acceptor
...
\hypertarget{denit}{\subsection{\texorpdfstring{A \emph{denitrification}
layer below the aerobic
layer}{A denitrification layer below the aerobic layer}}\label{denit}}
...
Nitrous oxide is thus a sign of an incomplete denitrification. Because
...
about finding ways to have have denitrification go all the way to the
...
electrons) in denitrification is illustrated in equation
...
nitrification can take place, as we shall see later.

Following the same analysis as that of aerobic respiration, because of
the transport distance for nitrate from the water column and through the
aerobic layer of the sediment, the supply tends to be lower than the
demand. This, with the demand below the supply, creates a concentration
gradient, which generates a \textbf{downward flux} of nitrate from the
water column through the aerobic zone of the sediment into the
denitrification zone (layer 2 in Figure \ref{fig:wetland-soil-layers}).
Nitrate thus just diffuse through layer 1, and is only used as electron
acceptor in layer 2. Because the demand exceeds the supply, nitrate
cannot move further down than the bottom of layer 2. Denitrification is
thus restricted to layer 2.

\subsection{Manganese and Iron oxides
...
gas, NO\textsubscript{3}\textsuperscript{-}, which is a dissolved ion,
...
general equation \eqref{eq:MnO2-reduc-half-reaction} and respiration
scheme illustrated in Figure \ref{fig:MnO2-resp}.

\begin{equation}
MnO_2 + 4 e^- + 4 H^+ \rightarrow Mn^{2+} + 2 H_2O
...
\begin{align}
Fe^{3+} + 1 e^- & \rightarrow & Fe^{2+} \\
Fe(OH)_3 + 1 e^- + 3 H^+ & \rightarrow & Fe^{2+} + 3 H_2O
\label{eq:Fe-reduc-half-reaction}
\end{align}

The overall transfer of electrons from the organic matter can be written
as:
...
a no net downward water infiltration, the Fe\textsuperscript{2+} and
...
\subsection{Sulfate reduction}\label{sulfate-reduction}
...
\caption{Respiration scheme for heterotrophic Iron oxide reduction}\label{fig:SO4-resp}
\end{figure}
...
there will be no sulfate any more.
...
\item
  ...
  \item
    then one add the appropriate number of H\textsuperscript{+} to
    balance the H
  \item
...
\begin{equation}
CO_2 + 4 H^+ + 4 e^- \rightarrow CH_2O + H_2O  
\end{equation}
...
\begin{equation}
CO_2 + 8 e^- + 8 H^+ \rightarrow CH_4 + 2 H_2O
\end{equation}
