...
O\textsubscript{2} the electron acceptor. Organisms which use
\emph{organic} molecules as electron donors are referred to as
organotrophs, hence the title. We shall later that some microorganisms
are able to use inorganic molecules as their electron donors. In our
case (Figure \ref{fig:aerobic-resp-scheme-glucose}), the byproduct of
glucose as the carbon atoms lose all their electrons becomes
CO\textsubscript{2}.
...
available for another oxidation to take place. Lithotroph
micro-organisms are specialized in taking advantage of these available
electrons, as we shall see later. The important message here is that you
can see that respiration processes essentially breakdown large molecules
into small inorganic ones, and that in the case of respiration for
organotrophs, the only atom to lose its electrons is carbon.
...
of organotroph respiration. Again, several group of
\protect\hyperlink{litrotrophs}{lithotrophs} specifically target the
electrons on ammonium and on Hydrogen sulfide, and use
O\textsubscript{2} as their electron acceptor.
...
\caption{Theoretical spatial layering of wetland soils corresponding to the electron acceptor available. In each layer, the oxidizing and the reduced forms are illustrated as oxidizing/reduced. *Not to scale*}\label{fig:wetland-soil-layers}
\end{figure}

It is now time to present the respiration processes in each of the
\emph{redox} layer.

\subsection{\texorpdfstring{A \emph{denitrification} layer below the
aerobic
layer}{A denitrification layer below the aerobic layer}}\label{a-denitrification-layer-below-the-aerobic-layer}
...
accept them to generate their energy.
